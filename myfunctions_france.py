...
##Bi-GRU
def bigru_model(new_data,i,look_back,data_partition,cap):
    ...
    neuron=128
    model = Sequential()
    model.add(Bidirectional(GRU(units = neuron),input_shape=(trainX1.shape[1], trainX1.shape[2])))
    model.add(Dense(1))
    optimizer = tf.keras.optimizers.Adam(learning_rate=0.001)
    model.compile(loss='mse',optimizer=optimizer)
...
##Proposed Method Hybrid CEEMDAN-EWT với GRU (Enhanced Version)
##Hướng phát triển mới: Thay LSTM bằng GRU, hỗ trợ GPU

def ceemdan_ewt_evl(new_data,i,look_back,data_partition,cap):
    ...
    for col in new_ceemdan:
        ...
        from keras.models import Sequential
        from keras.layers.core import Dense, Dropout, Activation
        from keras.layers import GRU  # Thay LSTM bằng GRU


        neuron=128
        model = Sequential()
        model.add(GRU(units = neuron,input_shape=(trainX.shape[1], trainX.shape[2])))  # Sử dụng GRU
        model.add(Dense(1))
        optimizer = tf.keras.optimizers.Adam(learning_rate=0.001)
        model.compile(loss='mse',optimizer=optimizer)
...
##Proposed Method Hybrid CEEMDAN-EWT với Bidirectional LSTM
##Hướng phát triển mới: Thay LSTM bằng Bidirectional LSTM, hỗ trợ GPU

def ceemdan_ewt_bilstm(new_data,i,look_back,data_partition,cap):
    ...
    for col in new_ceemdan:
        ...
        from keras.models import Sequential
        from keras.layers.core import Dense, Dropout, Activation
        from keras.layers import LSTM, Bidirectional  # Sử dụng Bidirectional LSTM


        neuron=128
        model = Sequential()
        model.add(Bidirectional(LSTM(units = neuron),input_shape=(trainX.shape[1], trainX.shape[2])))  # Bidirectional LSTM
        model.add(Dense(1))
        optimizer = tf.keras.optimizers.Adam(learning_rate=0.001)
        model.compile(loss='mse',optimizer=optimizer)
...
##Proposed Method Hybrid CEEMDAN-EWT với Bidirectional GRU
def ceemdan_ewt_bigru(new_data,i,look_back,data_partition,cap):
    ...
    for col in new_ceemdan:
        ...
        neuron=128
        model = Sequential()
        model.add(Bidirectional(GRU(units = neuron),input_shape=(trainX.shape[1], trainX.shape[2])))
        model.add(Dense(1))
        optimizer = tf.keras.optimizers.Adam(learning_rate=0.001)
        model.compile(loss='mse',optimizer=optimizer)
...
##Proposed Method Hybrid CEEMDAN-EWT với Transformer
##Hướng phát triển mới: Thay LSTM bằng Transformer Encoder, hỗ trợ GPU

def ceemdan_ewt_transformer(new_data,i,look_back,data_partition,cap):
    ...
    for col in new_ceemdan:
        ...
        from keras.layers.core import Dense, Dropout, Activation
        from tensorflow.keras.layers import Input, Dense as DenseLayer, LayerNormalization, GlobalAveragePooling1D
        
        # Transformer Encoder Architecture (simplified for TF 2.3 compatibility)
        # Sử dụng Self-Attention đơn giản với Dense layers
        inputs = Input(shape=(trainX.shape[1], trainX.shape[2]))
        
        # Self-attention mechanism (simplified)
        # Query, Key, Value projections
        query = DenseLayer(32)(inputs)
        key = DenseLayer(32)(inputs)
        value = DenseLayer(32)(inputs)
        
        # Attention scores
        attention_scores = tf.matmul(query, key, transpose_b=True)
        attention_scores = tf.nn.softmax(attention_scores / tf.sqrt(32.0))
        attention_output = tf.matmul(attention_scores, value)
        attention_output = Dropout(0.1)(attention_output)
        out1 = LayerNormalization(epsilon=1e-6)(inputs + attention_output)
        
        # Feed forward network
        ffn_output = DenseLayer(128, activation='relu')(out1)
        ffn_output = DenseLayer(trainX.shape[2])(ffn_output)
        ffn_output = Dropout(0.1)(ffn_output)
        out2 = LayerNormalization(epsilon=1e-6)(out1 + ffn_output)
        
        # Global average pooling và output
        pooled = GlobalAveragePooling1D()(out2)
        outputs = DenseLayer(1)(pooled)
...
# ---------------------------------------------------------------------------
# CEEMDAN-EWT + CNN variants (France dataset)

def _ceemdan_ewt_cnn_pipeline_fr(new_data,i,look_back,data_partition,cap,build_model_fn):
    ...
    if not pred_test:
        raise ValueError("CEEMDAN-EWT-CNN (France): không đủ dữ liệu để huấn luyện.")
...
def ceemdan_ewt_cnn(new_data,i,look_back,data_partition,cap):
    ...
    return _ceemdan_ewt_cnn_pipeline_fr(new_data,i,look_back,data_partition,cap,build_model)
